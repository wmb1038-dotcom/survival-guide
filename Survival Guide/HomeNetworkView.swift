...
struct HomeNetworkView: View {
    ...
    private let sections: [NetSectionItem] = [
        ...
        NetSectionItem(
            ...
            steps: [
                Step(n: "Option A", title: "Python 3 (built-in on macOS)", detail: "cd /Volumes/20TB_HDD/offline-library\npython3 -m http.server 8000 --bind 192.168.86.100\n\nReplace 192.168.86.100 with your Mac's actual LAN IP.\nThe --bind flag limits the server to the local mesh network only, blocking access from any other interface (e.g., cellular hotspot).\n\nAccess from any device: http://192.168.86.100:8000"),
